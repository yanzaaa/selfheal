...
The orchestration is a **UiPath coded agent** (`uipath-agent/`, Python — scaffolded, run & published with the `uipath` CLI; it calls UiPath **Identity** + **Test Manager v2** REST APIs directly): `select → generate → run → triage → heal/file-defect → report`. It runs via the UiPath runtime and is **published to the Orchestrator tenant feed** (`uipath pack` + `uipath publish --tenant`).
